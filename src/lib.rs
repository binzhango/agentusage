...
pub fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Command::Sync(args)) => {
            let provider = args.provider.unwrap_or_else(|| "codex".to_owned());
...
fn prepare_report_backend(provider: &str) -> Result<storage::BackendMode> {
    storage::prepare_backend_for_agent(true, provider)
}
...
fn ingest_provider(
    ...
    let (files_scanned, files_with_usage, token_records, malformed_lines) = match provider {
        ...
        "claude_code" => providers::local::ingest_into_store(
            providers::local::Agent::ClaudeCode,
            sessions_dir,
            store,
        )?,
        "opencode" => providers::local::ingest_into_store(
...
fn validate_provider(provider: &str) -> Result<()> {
    match provider {
        "codex" | "claude_code" | "opencode" | "copilot" | "pi" => Ok(()),
        other => {
            anyhow::bail!(
...
fn cache_hit_rate(report: &providers::codex::DailyUsage) -> Option<f64> {
    core::token_semantics_for_agent(&report.provider).cache_hit_rate(
        report.input_tokens,
